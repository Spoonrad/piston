...
if path.endswith('.py') and path.startswith('src/'):
  ...
  for _, c in inspect.getmembers(module):
    if inspect.isclass(c) and not inspect.isabstract(c) and issubclass(c, piston.Control):
      controls.append(c)
  p = piston.Piston(controls)
  failures, tests = doctest.testmod(module, globs={'piston': p.apply})
else:
  failures, tests = doctest.testfile(sys.argv[1], globs={"piston": piston.piston})
